...
import com.example.robot.Make;
import com.example.robot.Robot;
import com.example.robot.RobotStatus;
...
public class ServerProtocol {
    ...
    public static String processRequest(String messageFromClient, SimpleServer server) throws IOException {
        RequestParser parser = new RequestParser(messageFromClient);
     ...
     */
    private static String launch(RequestParser parser, SimpleServer server) throws IOException {
        String robot_name = parser.getName();
        TextWorld world = server.getWorld();
     ...
     * @return The response to the client.
     */
    private static String forwardBack(RequestParser parser, SimpleServer server) {
     ...
     * @return The response to the client.
     */
    private static String turn(RequestParser parser, SimpleServer server) {
        Robot robot = server.getRobot();
        TextWorld world = server.getWorld();
     ...
     * @return The response to the client.
     */
    static String state(RequestParser parser, SimpleServer server) {
        if(hasArgs(parser)){
            return InvalidResponse.unsupportedArgumentsResponse();
        }
     ...
     * @return The response to the client.
     */
    private static String look(RequestParser parser, SimpleServer server) {
        if(hasArgs(parser)){
            return InvalidResponse.unsupportedArgumentsResponse();
        }
        Robot robot = server.getRobot();
     ...
     * @return The response to the client.
     */
    private static String repair(RequestParser parser, SimpleServer server) {
        Robot robot = server.getRobot();
     ...
     * @return The response to the client.
     */
    private static String reload(RequestParser parser, SimpleServer server) {
        Robot robot = server.getRobot();
        if (robot.getStatus() == RobotStatus.NORMAL ) {
            CompletableFuture.supplyAsync(() -> {
                try {
     ...
     * @throws JsonProcessingException If there is an error processing JSON data.
     */
    private static String fire(RequestParser parser, SimpleServer server) throws JsonProcessingException {
        Robot robot = server.getRobot();
    ...
    private static String help(RequestParser parser, SimpleServer server)  {
     ...
     */
    private static Boolean hasArgs(RequestParser parser ){
